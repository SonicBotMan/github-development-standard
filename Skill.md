...
### Step 5：编码（Karpathy 四大原则 + 8 条纪律）

#### Karpathy 四大原则（源自 Andrej Karpathy 的 LLM 编程观察）

##### 原则 1：先思考，再动手（Think Before Coding）
> 不瞎猜。不掩盖困惑。主动暴露权衡。

- 不确定时，先说假设，别闷头猜
- 有多种理解时，列出所有方案，别偷偷选一个
- 存在更简单方案时，主动说出来
- 搞不清楚时，停下来问，别硬上
...
- 不要加没被要求的"灵活性"或"可配置性"
- 不要为不可能发生的场景写错误处理
- 200 行能搞定的事别写 50 行以上（如果 50 行够用就重写）
