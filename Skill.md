...
# 或使用 API（避免某些错误）
gh api repos/owner/repo/issues/53 --jq '{title, body, state, user: .user.login}'
```
...
1. **先检查所有相关文件**
   ```bash
   # 检查所有 README 文件是否有相同问题
   grep -n "问题关键词" README*.md
   ```

2. **同步修复**
   - 修改 README.md 后，立即检查 README_CN.md, README_JP.md 等
   - 确保所有语言版本的修复一致

3. **验证所有文件**
   ```bash
   # 验证所有文件都已修复
   grep -c "问题关键词" README*.md
   ```
...
1. **先整合内容** - 将有价值的内容合并到 CHANGELOG.md 或其他文档
2. **再删除文件** - 确认整合后再删除冗余文件
3. **验证链接** - 确保没有死链接
...
```bash
# 检查常见冗余文件
ls -1 | grep -E "DUAL_RELEASE|STAR_HISTORY|RELEASE_NOTES|README_V5|DOWNLOAD_INSTRUCTIONS"
```
...
```bash
# 检查下载链接版本号
grep -o "releases/tag/v[0-9.]*)" README*.md | sort | uniq -c | awk '$1 > 1 {print "⚠️  版本号不一致"}'
```
...
- `examples/openclaw-portable-issue53.md` - **OpenClaw Portable 文档修复案例（新增）**
